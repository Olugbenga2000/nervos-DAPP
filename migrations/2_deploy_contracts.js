...
module.exports = async function(deployer, _network, accounts) {
  ...
  await Promise.all(
      [Dai,Zrx,Bat,Rep,Dex].map(contract => deployer.deploy(contract)))
  const [dai,zrx,bat,rep,dex] = await Promise.all(
        [Dai,Zrx,Bat,Rep,Dex].map(contract => contract.deployed()))
  await Promise.all([
    ...
    await Promise.all([dai,zrx,bat,rep].map(token => seedTokenBalance(token,trader3)))
    await Promise.all([dai,zrx,bat,rep].map(token => seedTokenBalance(token,trader4)))
